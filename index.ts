...
const embedPayload = async (archive: JSZip) => {
  if (
    !await file(PAYLOAD_DIR).stat().then((stats) => stats.isDirectory()).catch(
      () => false,
    )
  ) {
    console.log(`PAYLOAD_DIR '${PAYLOAD_DIR}' not found, skipping embed`);
    return;
  }
...
if (import.meta.main) {
  ...
  try {
    ...
    if (!parsed.success || parsed.data.status !== 404) {
      throw httpErrorSchema.safeParse(error);
    }
  }
  ...
  const version = parse(payloadJson.version, true)!.toString();
